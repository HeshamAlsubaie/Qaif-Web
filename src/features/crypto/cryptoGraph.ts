 ...
 *   - directional edges: each edge keeps its REAL money direction (source→target), so the drawn
 *     arrows show funds moving in AND out — money into the sanctioned origin is as much a part of the
 *     flow to explore as money out of it;
 *   - `childrenOf`/`parentsOf` (the two directions) PLUS `neighborsOf` (their union) — the reveal
 *     walks `neighborsOf`, so expanding a wallet surfaces every counterparty it transacted with,
 *     inflow OR outflow, not just the ones it happened to fund;
...
export interface CryptoGraphModel {
  ...
  /**
   * Direction-AGNOSTIC counterparties: `childrenOf ∪ parentsOf`, deduped. The reveal/expand
   * traversal walks THIS (not childrenOf), so a wallet reveals every counterparty it transacted
   * with in either direction — the origin, mostly a TARGET of inflows, still reveals its senders.
   */
...
/**
 * The set of node ids currently VISIBLE given which nodes are expanded. Progressive reveal: the
 * roots (and thus, since roots start expanded, their hop-1 counterparties) are shown; a deeper node
 * appears only once a visible node is expanded. The walk follows `neighborsOf` (both directions), so
 * the origin — mostly a TARGET of inflows — still reveals every wallet it transacted with, not just
 * the few it funded. This keeps a 275-node trace legible: you explore it counterparty by
 * counterparty instead of dumping every node at once.
 */
export function computeVisibleNodes(
  ...
  while (stack.length > 0) {
    ...
    for (const c of model.neighborsOf.get(n) ?? []) {
      if (!visible.has(c)) {
        visible.add(c);
        stack.push(c);
      }
...
/** How many of a node's counterparties (either direction) are hidden — drives the `+N` affordance. */
export function hiddenChildCount(
  model: CryptoGraphModel,
  entityId: number,
  visible: ReadonlySet<number>,
): number {
  const neighbors = model.neighborsOf.get(entityId);
  if (!neighbors) return 0;
  return neighbors.reduce((acc, c) => (visible.has(c) ? acc : acc + 1), 0);
}
